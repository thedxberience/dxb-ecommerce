...
function buildProductQuery(filters: SanityProductFilters) {
  ...
  const filterClause = conditions.length > 0
    ? `[_type == "Product" && (${conditions.join(' || ')}) && defined(asset.image.asset->url)${lastIdConition}]`
...
export const getAllSanityProductsByFilterCount = async (filters: SanityProductFilters) => {
  const {
    category,
    subCategory,
    brand,
  ...
  const query = `count(*[_type == "Product" && (${conditions.join(' || ')}) && asset.image.asset->url != null])`;
  // console.log("Sanity Query for Count:", query);
  const { data: count, error: fetchSanityProductsErr } = await tryCatch(sanityClient.fetch(query))
  if (fetchSanityProductsErr) {
    console.error("Error fetching product count from Sanity:", fetchSanityProductsErr)
    return {
      data: null,
...
export const getAllSanityProductsByFilters = async (filters: SanityProductFilters) => {
